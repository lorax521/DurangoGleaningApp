    ...
    $.getJSON(`https://${cartoDBUserName}.carto.com/api/v2/sql?format=GeoJSON&q=${sqlQueryClosest}`, function(data) {
      markerLocations = L.geoJson(data, {
        onEachFeature: function (feature, layer) {
          ...
          layer.bindPopup(` ${feature.properties.type} ${feature.properties.count} ${feature.properties.permission} ${feature.properties.notes} `);
          layer.cartodb_id=feature.properties.cartodb_id;
        }
      }).addTo(map);
    });
...
let customMarker = new myCustomMarker();
...
function toggleEdit() {
  ...
  function cancelPostPoint() {
    ...
    setTimeout(showAllWithEdit(), 6000);
  }
  ...
  function postPoint(layer) {
    ...
    function persistOnCartoDB(action, layers) {
      ...
      */
      ...
      switch (action) {
        ...
        case "DELETE":
          // layers.eachLayer(function(layer) {
            ...
            // cartodb_ids.push(cartoID); //trying this
            geojsons.push("''");
            // });
          break;
      }
      ...
      sql += "],ARRAY[";
      sql += geojsons.join(",");
      sql += "],ARRAY['"+ Vtype + "'],ARRAY['"+ Vcount + "'],ARRAY['"+ Vpermission + "'],ARRAY['"+ Vnotes + "']);";
    ...
    function deletePoint() {
      if(confirm('Are you sure you want to delete this point? THIS CANNOT BE UNDONE')) {
        ...
        // if(!functionComplete) { //https://stackoverflow.com/questions/21518381/proper-way-to-wait-for-one-function-to-finish-before-continuing
        //  setTimeout(location.reload(), 3000);
         setTimeout(showAllWithEdit(), 6000);
      }
    };
